...
public class LogUtil01 {
    ...
    @Before("execution(public int com.youyoustudio.impl.MyCalculator01.*(int,int))")
    public void logStart() {
        System.out.println("[XXXX]方法开始执行，用的参数列表是[XXXX]");
    }

    //目标方法正常执行完成之后执行
    @AfterReturning("execution(public int com.youyoustudio.impl.MyCalculator01.*(int,int))")
    public void logReturn() {
        System.out.println("[XXXX]方法正常执行完成，计算结果是：");
    }

    //目标方法出现异常
    @AfterThrowing("execution(public int com.youyoustudio.impl.MyCalculator01.*(int,int))")
    public void logException() {
        System.out.println("[XXXX]方法执行出现异常，异常信息：");
    }

    //目标方法最终结束之前
    @After("execution(public int com.youyoustudio.impl.MyCalculator01.*(int,int))")
    public void logEnd() {
